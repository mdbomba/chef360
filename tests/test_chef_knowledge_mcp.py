...
class ChefKnowledgeMcpTests(unittest.TestCase):
    ...
    def test_public_search_finds_provider_neutral_installation(self) -> None:
        ...
        self.assertTrue(
            any(match["id"] == "operations-infrastructure-automation" for match in result["matches"])
        )

    def test_public_search_finds_kvm_autoinstall_troubleshooting(self) -> None:
        result = self.public_knowledge.search(
            {"query": "autoinstall Netplan ftype Mailpit", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "operations-kvm-lab-deployment" for match in result["matches"])
        )

    def test_public_search_finds_config_values_installation(self) -> None:
        result = self.public_knowledge.search(
            {"query": "--config-values", "limit": 5}
        )
        self.assertTrue(
    ...
    def test_public_search_finds_automate_connector_configuration(self) -> None:
        result = self.public_knowledge.search(
            {"query": "Automate Connector Data Collector API Key", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "chef-automate-overview" for match in result["matches"])
        )

    def test_public_search_finds_automate_connector_lifecycle(self) -> None:
        result = self.public_knowledge.search(
            {"query": "connector lifecycle future integrated capabilities", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "chef-automate-overview" for match in result["matches"])
        )

    def test_public_search_finds_automate_connector_pod_dns_guidance(self) -> None:
        result = self.public_knowledge.search(
            {"query": "CoreDNS private lab Automate", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "chef-automate-overview" for match in result["matches"])
        )

    def test_public_search_finds_libvirt_dns_preflight(self) -> None:
        result = self.public_knowledge.search(
            {"query": "libvirt dnsmasq public upstream", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "operations-kvm-lab-deployment" for match in result["matches"])
        )

    def test_public_search_finds_validated_automate_fqdn(self) -> None:
        result = self.public_knowledge.search(
            {"query": "500 query CoreDNS burn test automate FQDN", "limit": 5}
        )
        self.assertTrue(
            any(match["id"] == "chef-automate-overview" for match in result["matches"])
        )
    ...
    def test_initialize_and_tools_list(self) -> None:
        ...
        self.assertLessEqual(len(instructions), 350)
        self.assertIn("Read-only", instructions)
        self.assertIn("Chef 360 1.7.3", instructions)
        ...
        if self.knowledge is None:
            self.assertNotIn("search_chef_repositories", tool_names)
        if self.corpus is None:
            self.assertNotIn("search_chef_knowledge", tool_names)

    def test_service_overview_is_brief_by_default(self) -> None:
        overview = self.server.call_tool("get_chef_service_overview", {})["structuredContent"]
        self.assertTrue(overview["service"]["read_only"])
        self.assertEqual("1.7.3", overview["knowledge"]["version"])
        self.assertNotIn("sources", overview)
        self.assertNotIn("capabilities", overview)

        sources = self.server.call_tool(
            "get_chef_service_overview", {"detail": "sources"}
        )["structuredContent"]
        self.assertIn("sources", sources)

    def test_public_only_discovery_hides_unavailable_sources(self) -> None:
        ...
        self.assertEqual(4, len(tools["result"]["tools"]))
        self.assertTrue(
            all(not item["uri"].startswith("chef-cft://") for item in resources["result"]["resources"])
        )
        self.assertTrue(
            any(item["uri"] == "chef360://service/overview" for item in resources["result"]["resources"])
        )
...
class LabToolsTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.lab = LabInspector()

    def test_lab_machines_are_allowlisted(self) -> None:
        self.assertEqual(
            sorted(LAB_MACHINES),
            ["automate", "chef360", "node1", "node2"],
        )
        self.assertEqual(LAB_MACHINES["chef360"]["domain"], "20_chef360")
        self.assertEqual(LAB_MACHINES["chef360"]["servicePort"], 31000)
        self.assertEqual(LAB_MACHINES["automate"]["ip"], "10.0.0.21")

    def test_list_machines_returns_all(self) -> None:
        machines = self.lab.list_machines()
        self.assertEqual(sorted(machines), sorted(LAB_MACHINES))

    def test_inspect_machine_includes_state_fields(self) -> None:
        result = self.lab.inspect_machine("chef360")
        self.assertEqual(result["name"], "chef360")
        self.assertIn("vmState", result)
        self.assertIn("serviceReachable", result)
        self.assertIsInstance(result["serviceReachable"], bool)

    def test_inspect_unknown_machine_raises(self) -> None:
        with self.assertRaises(ToolError):
            self.lab.inspect_machine("unknown")

    def test_workstation_versions_returns_text(self) -> None:
        result = self.lab.workstation_versions()
        self.assertIsInstance(result, str)
        self.assertTrue(result.strip())

    def test_server_exposes_lab_tools(self) -> None:
        server = McpServer(Chef360KnowledgeSet(), lab=LabInspector())
        machines = server.call_tool("list_lab_machines", {})["structuredContent"]
        self.assertEqual(sorted(machines), sorted(LAB_MACHINES))
        inspected = server.call_tool("inspect_lab_machine", {"machine": "automate"})["structuredContent"]
        self.assertEqual(inspected["name"], "automate")
        versions = server.call_tool("chef_workstation_versions", {})["content"][0]["text"]
        self.assertIsInstance(versions, str)
        self.assertTrue(versions.strip())
