...
def main() -> int:
    ...
    try:
        ...
        instructions = initialized.get("result", {}).get("instructions", "")
        if "Read-only" not in instructions or len(instructions) > 350:
            raise RuntimeError(f"unexpected onboarding instructions: {instructions}")
        ...
        if DEFAULT_CORPUS.is_file():
            expected.update({"search_chef_knowledge", "get_chef_document"})
        ...
        if "chef360://service/overview" not in resource_uris:
            raise RuntimeError("service overview resource is unavailable")
        if not source_summary["metadata_index"].get("available", True) and any(
            uri.startswith("chef-cft://") and uri != "chef-cft://knowledge/corpus-stats"
            for uri in resource_uris
        ):
            raise RuntimeError("unavailable protected metadata resources were advertised")
