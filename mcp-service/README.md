...
On connection, the service gives a short read-only introduction and recommends
a search-first workflow. Call `get_chef_service_overview` for an explicit brief;
request source or capability detail only when needed. The service does not load
the complete knowledge set into the conversation.
...
- `search_knowledge`: ranked search over manifest-declared Chef 360 1.7.3 documents.
- `get_chef360_document`: bounded retrieval of a selected document.
- `get_chef_service_overview`: brief-by-default identity and capability overview.

The service also exposes versioned overview, manifest, and README resources.
...
Lab tools are registered only when both `virsh` and Chef Workstation are
available on the MCP host. This keeps hosted and public-only capability lists
accurate.
