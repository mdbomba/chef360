...
- Create separate `/boot`, `/`, and `/var/lib` filesystems. This guide recommends ext4 for `/var/lib`; XFS with `ftype=1` is also acceptable.
- Ensure `/var` is not mounted with the `noexec` option.
- Do not configure a swap partition or a swap file on `/` or `/var/lib`.
...
Use `scripts/chef360/install-server.sh` from this repository after download.
It supports a traditional install without ConfigValues and a repeatable install
with a protected ConfigValues export:
