...
require_text "$HELPER" 'AIRGAP=false' 'helper has a visible air-gap selection parameter'
require_text "$HELPER" 'CHECK_LATEST=true' 'helper checks the latest documented release'
require_text "$HELPER" 'No installation was started.' 'helper stops after package extraction'
...
require_text "$EXPORT_GUIDE" 'docker.io/progressofficial/chef360:1.0.3' 'exportable guide includes the PostgreSQL Velero plugin image'
require_text "$EXPORT_GUIDE" 'docker.io/velero/velero-plugin-for-aws:v1.12.1' 'exportable guide includes the AWS Velero plugin image'
require_text "$EXPORT_GUIDE" '/var/lib/embedded-cluster/k0s/images/' 'exportable guide includes the air-gap image preload path'
