...
def gh_api(endpoint: str) -> Any:
    ...
    if "GH_TOKEN" not in environment:
        token = load_secret("GH_TOKEN", fallback_path=TOKEN_PATH)
        if not token:
            raise RuntimeError(
                f"GitHub token not found: set GH_TOKEN in the environment, "
                f"create {TOKEN_PATH}, or export GH_TOKEN in ~/.bashrc"
            )
