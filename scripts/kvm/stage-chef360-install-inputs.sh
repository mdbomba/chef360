...
for file in \
  ...
  "${CHEF360_STAGE_DIR}/${VM_NAME}-config.yaml" \
  "${CHEF360_STAGE_DIR}/tls/${VM_NAME}.crt" \
  "${CHEF360_STAGE_DIR}/tls/${VM_NAME}.key" \
  "${CHEF360_STAGE_DIR}/tls/${VM_NAME}.chain.crt" \
  "${CHEF360_STAGE_DIR}/ca/${VM_NAME}-ica.crt" \
  "${CHEF360_STAGE_DIR}/ca/${VM_NAME}-rca.crt"; do
  require_file "${file}"
done
...
ssh "${ssh_args[@]}" "${target}" "sudo -n env REMOTE_STAGE='${remote_stage}' VM_NAME='${VM_NAME}' sh -eu" <<'REMOTE'
...
install -m 0600 "$REMOTE_STAGE/${VM_NAME}-config.yaml" /opt/chef360/chef-config.yaml
install -m 0644 "$REMOTE_STAGE/tls/${VM_NAME}.crt" /opt/chef360/tls/chef360-2.crt
install -m 0600 "$REMOTE_STAGE/tls/${VM_NAME}.key" /opt/chef360/tls/chef360-2.key
install -m 0644 "$REMOTE_STAGE/tls/${VM_NAME}.chain.crt" /opt/chef360/tls/chef360-2.chain.crt
install -m 0644 "$REMOTE_STAGE/ca/${VM_NAME}-ica.crt" /usr/local/share/ca-certificates/${VM_NAME}-ica.crt
install -m 0644 "$REMOTE_STAGE/ca/${VM_NAME}-rca.crt" /usr/local/share/ca-certificates/${VM_NAME}-rca.crt
